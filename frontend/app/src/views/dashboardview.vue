  ...
  <div id="app">
    ...
    <div class="flex flex-1">
      ...
      <div
        class="fixed top-12 left-0 w-64 h-[calc(100vh-65px)] bg-gray-100 p-4 overflow-y-auto mt-4 z-50 transform md:transform-none transition-transform duration-300 ease-in-out"
        :class="{ '-translate-x-full': !isMenuOpen, 'translate-x-0': isMenuOpen }"
        v-if="currentSection === 'AITools'">
        <a href="#" @click.prevent="navigate('/ai-tools/generator')"
          :class="{ 'text-green-800 font-semibold': isActive('/ai-tools/generator'), 'hover:bg-gray-200 hover:font-semibold': !isActive('/ai-tools/generator') }"
          class="block p-3 text-gray-600 rounded-lg ">
          <i class="bi bi-magic mr-2"></i>Message Generator
        </a>
      </div>
  ...
  data() {
    return {
      ...
      navItems: [
        ...
        // NEW: Add AI Tools to the main navigation
        { name: 'AITools', label: 'AI Tools', icon: 'bi bi-stars', path: '/ai-tools/generator' },
      ],
      user: null,
      dropdownOpen: false,
...
function getSectionFromRoute(path) {
  ...
  if (path.startsWith('/ai-tools')) return 'AITools'; // NEW: Handle the new AI Tools route
